...
class AgentFactory:
    def __init__(self):
        ...
        self.model_name = os.getenv("MODEL_NAME") or "gpt-3.5-turbo"
        self.base_url = 'http://localhost:11434/v1'
        self.model = OpenAIModel(model_name=self.model_name, 
                                 provider=OpenAIProvider(base_url=self.base_url))

    def create_agent(self) -> Agent[AgentDependencies, AgentResponse]:
        """Create a new AI agent."""
        AIAgent = Agent[AgentDependencies, AgentResponse](
            model=self.model,
